...
DOCUMENTATION = '''
---
module: niftycloud_lb_filter
short_description: Apply filter settings to Load Balancer in NIFTY Cloud
description:
	- Apply filter settings to Load Balancer.
version_added: "0.1"
options:
	access_key:
		description:
    ...
    loadbalancer_port:
        ...
        required: false
		default: null
	state:get_state_instance_in_load_balancer
		description:
...
## DescribeLoadBalancers の情報からInstanceIDのリストを取ってきてあるかどうかを判定する
def get_state_instance_in_load_balancer(module):
    ...
    if res['status'] == 200:
        for instance_id in res['xml_body'].findall(
                './/{{{nc}}}Instances/{{{nc}}}member/{{{nc}}}InstanceId'.format(**res['xml_namespace'])):
...
def deregister_filter(module):
    ...
    if lbs_res['status'] == 200:
        changed = False
        for member in lbs_res['xml_body'].findall(
                './/{{{nc}}}LoadBalancerDescriptions/{{{nc}}}member'.format(**lbs_res['xml_namespace'])):
            if member.find('.//{{{nc}}}IPAddress'.format(**lbs_res['xml_namespace'])) is None:
                continue

            deregister_ips = [x.text for x in member.findall('.//{{{nc}}}IPAddress'.format(**lbs_res['xml_namespace']))]
            instance_port = member.find('.//{{{nc}}}InstancePort'.format(**lbs_res['xml_namespace'])).text

            if instance_port == None:
                pass # エラー処理

            # 許可IPが空の場合何もしない
            if len(deregister_ips) == 1 and deregister_ips[0] == '*.*.*.*':
                continue

            params['InstancePort'] = instance_port
            for index, value in enumerate(deregister_ips):
                params['IPAddresses.member.' + str(index) + '.IPAddress'] = value

            res = request_to_api(module, 'GET', 'SetFilterForLoadBalancer', params)
...
def main():
    ...
    module_obj = lambda **args: type('<anonymous object>', (object, ), args)()
    module = module_obj(
        params = {
    ...
    if goal_state == 'present':
        (changed, current_state) = regist_filter(module)
    elif goal_state == 'absent':
        (changed, current_state) = deregister_filter(module)
    else:
